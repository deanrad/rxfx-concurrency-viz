...
import { bus } from './bus';
import { queueOnlyLatest, toggleMap } from '@rxfx/operators';
import { reducer, GraphShape, BlockDisplay } from './blockService.reducer';
import { SINGLE_DURATION } from './constants';
import { animationService } from './animationService';
import { combineLatest, fromEvent, pipe, Subscription } from 'rxjs';
import {
  scan,
  takeUntil,
  mergeMap,
...
switch (q) {
  case 'queueing':
    ...
    break;
  case 'replacing':
    serviceOp = switchMap; // replacing
    break;
  case 'blocking':
    serviceOp = exhaustMap; // blocking
    break;
  case 'toggling':
...
bus.query(blockService.actions.request.match).subscribe({
  next(r) {
